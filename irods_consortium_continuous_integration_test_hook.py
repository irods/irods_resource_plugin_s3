...
def install_test_prerequisites():
    irods_python_ci_utilities.subprocess_get_output(['sudo', 'python2', '-m', 'pip', 'install', '--upgrade', 'pip==20.3.4'], check_rc=True)
    ...
    if options.do_setup:
        irods_python_ci_utilities.install_os_packages_from_files(
            glob.glob(os.path.join(os_specific_directory,
                      'irods-resource-plugin-s3*.{}'.format(package_suffix))
            )
        )
