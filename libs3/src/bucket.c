...
static S3Status testBucketPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	TestBucketData* tbData = (TestBucketData*) callbackData;

	return (*(tbData->responsePropertiesCallback))(responseProperties, tbData->callbackData);
}

static S3Status testBucketDataCallback(int bufferSize, const char* buffer, void* callbackData)
{
	TestBucketData* tbData = (TestBucketData*) callbackData;
...
static void testBucketCompleteCallback(S3Status requestStatus, const S3ErrorDetails* s3ErrorDetails, void* callbackData)
{
	TestBucketData* tbData = (TestBucketData*) callbackData;

	// Copy the location constraint into the return buffer
	snprintf(tbData->locationConstraintReturn, tbData->locationConstraintReturnSize, "%s", tbData->locationConstraint);

	(*(tbData->responseCompleteCallback))(requestStatus, s3ErrorDetails, tbData->callbackData);
...
static S3Status createBucketPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	CreateBucketData* cbData = (CreateBucketData*) callbackData;

	return (*(cbData->responsePropertiesCallback))(responseProperties, cbData->callbackData);
}

static int createBucketDataCallback(int bufferSize, char* buffer, void* callbackData)
{
	CreateBucketData* cbData = (CreateBucketData*) callbackData;
                                         ...
                                         void* callbackData)
{
	...
	(*(cbData->responseCompleteCallback))(requestStatus, s3ErrorDetails, cbData->callbackData);

	free(cbData);
}

static S3Status createBucketFromS3Callback(int bufferSize, const char* buffer, void* callbackData)
{
...
{
	// Create the callback data
	CreateBucketData* cbData = (CreateBucketData*) malloc(sizeof(CreateBucketData));
	if (!cbData) {
		(*(handler->completeCallback))(S3StatusOutOfMemory, 0, callbackData);
		return;
	}
	...
	if (locationConstraint) {
		cbData->docLen = snprintf(cbData->doc,
		                          sizeof(cbData->doc),
...
static S3Status deleteBucketPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	DeleteBucketData* dbData = (DeleteBucketData*) callbackData;

	return (*(dbData->responsePropertiesCallback))(responseProperties, dbData->callbackData);
}
                                         ...
                                         void* callbackData)
{
	...
	(*(dbData->responseCompleteCallback))(requestStatus, s3ErrorDetails, dbData->callbackData);
...
{
	// Create the callback data
	DeleteBucketData* dbData = (DeleteBucketData*) malloc(sizeof(DeleteBucketData));
	if (!dbData) {
		(*(handler->completeCallback))(S3StatusOutOfMemory, 0, callbackData);
		return;
	}
...
static S3Status make_list_bucket_callback(ListBucketData* lbData)
{
	...
	for (i = 0; i < contentsCount; i++) {
		...
		contentDest->ownerDisplayName = (contentSrc->ownerDisplayName[0] ? contentSrc->ownerDisplayName : 0);
	}
	...
	return (*(lbData->listBucketCallback))(isTruncated,
	                                       lbData->nextMarker,
	                                       contentsCount,
	                                       contents,
	                                       commonPrefixesCount,
...
static S3Status listBucketXmlCallback(const char* elementPath, const char* data, int dataLen, void* callbackData)
{
	...
	if (data) {
		...
		else if (!strcmp(elementPath, "ListBucketResult/Contents/LastModified")) {
			ListBucketContents* contents = &(lbData->contents[lbData->contentsCount]);
		...
		else if (!strcmp(elementPath, "ListBucketResult/Contents/Owner/DisplayName")) {
			ListBucketContents* contents = &(lbData->contents[lbData->contentsCount]);
			string_buffer_append(contents->ownerDisplayName, data, dataLen, fit);
		}
		else if (!strcmp(elementPath, "ListBucketResult/CommonPrefixes/Prefix")) {
			int which = lbData->commonPrefixesCount;
			size_t oldLen = lbData->commonPrefixLens[which];
			lbData->commonPrefixLens[which] += snprintf(lbData->commonPrefixes[which] + oldLen,
			                                            sizeof(lbData->commonPrefixes[which]) - oldLen - 1,
			                                            "%.*s",
			                                            dataLen,
			                                            data);
			if (lbData->commonPrefixLens[which] >= (int) sizeof(lbData->commonPrefixes[which])) {
		...
		if (!strcmp(elementPath, "ListBucketResult/Contents")) {
			...
			if (lbData->contentsCount == MAX_CONTENTS) {
				...
				initialize_list_bucket_contents(&(lbData->contents[lbData->contentsCount]));
			}
		}
		else if (!strcmp(elementPath, "ListBucketResult/CommonPrefixes/Prefix")) {
			// Finished a Prefix
...
static S3Status listBucketPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	ListBucketData* lbData = (ListBucketData*) callbackData;

	return (*(lbData->responsePropertiesCallback))(responseProperties, lbData->callbackData);
}

static S3Status listBucketDataCallback(int bufferSize, const char* buffer, void* callbackData)
{
	ListBucketData* lbData = (ListBucketData*) callbackData;
...
static void listBucketCompleteCallback(S3Status requestStatus, const S3ErrorDetails* s3ErrorDetails, void* callbackData)
{
	ListBucketData* lbData = (ListBucketData*) callbackData;
	...
	(*(lbData->responseCompleteCallback))(requestStatus, s3ErrorDetails, lbData->callbackData);
...
#define safe_append(name, value)                                                                             \
	do {                                                                                                     \
		...
		if (amp) {                                                                                           \
			...
			if (!fit) {                                                                                      \
				(*(handler->responseHandler.completeCallback))(S3StatusQueryParamsTooLong, 0, callbackData); \
				return;                                                                                      \
			}                                                                                                \
		}                                                                                                    \
	...
	lbData->responsePropertiesCallback = handler->responseHandler.propertiesCallback;
	lbData->listBucketCallback = handler->listBucketCallback;
	lbData->responseCompleteCallback = handler->responseHandler.completeCallback;
