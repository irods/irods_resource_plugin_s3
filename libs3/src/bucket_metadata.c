...
static S3Status getAclPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	GetAclData* gaData = (GetAclData*) callbackData;

	return (*(gaData->responsePropertiesCallback))(responseProperties, gaData->callbackData);
}

static S3Status getAclDataCallback(int bufferSize, const char* buffer, void* callbackData)
{
	GetAclData* gaData = (GetAclData*) callbackData;
...
static void getAclCompleteCallback(S3Status requestStatus, const S3ErrorDetails* s3ErrorDetails, void* callbackData)
{
	GetAclData* gaData = (GetAclData*) callbackData;

	if (requestStatus == S3StatusOK) {
		// Parse the document
		requestStatus = S3_convert_acl(gaData->xmlDocument,
		                               gaData->ownerId,
		                               gaData->ownerDisplayName,
		                               gaData->aclGrantCountReturn,
		                               gaData->aclGrants);
	}

	(*(gaData->responseCompleteCallback))(requestStatus, s3ErrorDetails, gaData->callbackData);
...
#define append(fmt, ...)                                                                     \
	...
	append("<AccessControlPolicy><Owner><ID>%s</ID><DisplayName>%s"
	       "</DisplayName></Owner><AccessControlList>",
	       ownerId,
	       ownerDisplayName);
...
static S3Status setXmlPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
...
static int setXmlDataCallback(int bufferSize, char* buffer, void* callbackData)
{
	...
	int remaining = (paData->xmlDocumentLen - paData->xmlDocumentBytesWritten);
	...
	memcpy(buffer, &(paData->xmlDocument[paData->xmlDocumentBytesWritten]), toCopy);
...
static void setXmlCompleteCallback(S3Status requestStatus, const S3ErrorDetails* s3ErrorDetails, void* callbackData)
{
	SetXmlData* paData = (SetXmlData*) callbackData;
                ...
                void* callbackData)
{
	...
	S3Status status = generateAclXmlDocument(
		ownerId, ownerDisplayName, aclGrantCount, aclGrants, &(data->xmlDocumentLen), aclBuffer, sizeof(aclBuffer));
...
static S3Status getLifecyclePropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
...
static S3Status getLifecycleDataCallback(int bufferSize, const char* buffer, void* callbackData)
{
	GetLifecycleData* gaData = (GetLifecycleData*) callbackData;
                      ...
                      void* callbackData)
{
	...
	generate_content_md5(data->xmlDocument, data->xmlDocumentLen, md5Base64, sizeof(md5Base64));
