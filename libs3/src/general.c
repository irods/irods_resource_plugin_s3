...
static S3Status convertAclXmlCallback(const char* elementPath, const char* data, int dataLen, void* callbackData)
{
	...
	if (data) {
		if (!strcmp(elementPath, "AccessControlPolicy/Owner/ID")) {
			caData->ownerIdLen += snprintf(&(caData->ownerId[caData->ownerIdLen]),
			...
			if (caData->ownerIdLen >= S3_MAX_GRANTEE_USER_ID_SIZE) {
				return S3StatusUserIdTooLong;
		                 ...
		                 "DisplayName"))
		{
			caData->ownerDisplayNameLen += snprintf(&(caData->ownerDisplayName[caData->ownerDisplayNameLen]),
			                                        S3_MAX_GRANTEE_DISPLAY_NAME_SIZE - caData->ownerDisplayNameLen - 1,
			                                        "%.*s",
			                                        dataLen,
			                                        data);
			if (caData->ownerDisplayNameLen >= S3_MAX_GRANTEE_DISPLAY_NAME_SIZE) {
		            ...
		            "Grant"))
		{
			...
			if (caData->emailAddress[0]) {
				...
				else if (!strcmp(caData->groupUri, ACS_GROUP_LOG_DELIVERY)) {
