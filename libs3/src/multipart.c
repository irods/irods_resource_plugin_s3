...
static S3Status initialMultipartXmlCallback(const char* elementPath, const char* data, int dataLen, void* callbackData)
{
	InitialMultipartData* mdata = (InitialMultipartData*) callbackData;
	int fit;
	if (data) {
...
static S3Status InitialMultipartPropertiesCallback(const S3ResponseProperties* properties, void* callbackData)
{
	InitialMultipartData* mdata = (InitialMultipartData*) callbackData;
...
static S3Status commitMultipartPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	CommitMultiPartData* data = (CommitMultiPartData*) callbackData;
...
static S3Status listMultipartPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	ListMultipartData* lmData = (ListMultipartData*) callbackData;

	return (*(lmData->responsePropertiesCallback))(responseProperties, lmData->callbackData);
}

static S3Status listPartsPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	ListPartsData* lpData = (ListPartsData*) callbackData;

	return (*(lpData->responsePropertiesCallback))(responseProperties, lpData->callbackData);
}

static S3Status listMultipartDataCallback(int bufferSize, const char* buffer, void* callbackData)
{
	ListMultipartData* lmData = (ListMultipartData*) callbackData;
...
static S3Status listPartsDataCallback(int bufferSize, const char* buffer, void* callbackData)
{
	ListPartsData* lpData = (ListPartsData*) callbackData;
...
static S3Status make_list_multipart_callback(ListMultipartData* lmData)
{
	...
	for (i = 0; i < uploadsCount; i++) {
		...
		uploadDest->ownerId = uploadSrc->ownerId[0] ? uploadSrc->ownerId : 0;
		uploadDest->ownerDisplayName = (uploadSrc->ownerDisplayName[0] ? uploadSrc->ownerDisplayName : 0);
	...
	return (*(lmData->listMultipartCallback))(isTruncated,
	                                          lmData->nextKeyMarker,
	                                          lmData->nextUploadIdMarker,
	                                          uploadsCount,
	                                          uploads,
	                                          commonPrefixesCount,
...
static S3Status make_list_parts_callback(ListPartsData* lpData)
{
	...
	return (*(lpData->listPartsCallback))(isTruncated,
	                                      lpData->nextPartNumberMarker,
	                                      lpData->initiatorId,
	                                      lpData->initiatorDisplayName,
	                                      lpData->ownerId,
	                                      lpData->ownerDisplayName,
	                                      lpData->storageClass,
	                                      partsCount,
	                                      lpData->handlePartsStart,
	                                      Parts,
	                                      lpData->callbackData);
}
                                          ...
                                          void* callbackData)
{
	...
	(*(lmData->responseCompleteCallback))(requestStatus, s3ErrorDetails, lmData->callbackData);
...
static void listPartsCompleteCallback(S3Status requestStatus, const S3ErrorDetails* s3ErrorDetails, void* callbackData)
{
	ListPartsData* lpData = (ListPartsData*) callbackData;
	...
	(*(lpData->responseCompleteCallback))(requestStatus, s3ErrorDetails, lpData->callbackData);
...
static S3Status listMultipartXmlCallback(const char* elementPath, const char* data, int dataLen, void* callbackData)
{
	ListMultipartData* lmData = (ListMultipartData*) callbackData;
	...
	if (data) {
		if (!strcmp(elementPath, "ListMultipartUploadsResult/IsTruncated")) {
			...
			ListMultipartUpload* uploads = &(lmData->uploads[lmData->uploadsCount]);
		...
		else if (!strcmp(elementPath, "ListMultipartUploadsResult/Upload/Owner/DisplayName")) {
			ListMultipartUpload* uploads = &(lmData->uploads[lmData->uploadsCount]);
			string_buffer_append(uploads->ownerDisplayName, data, dataLen, fit);
		}
		...
		else if (!strcmp(elementPath, "ListMultipartUploadsResult/CommonPrefixes/Prefix")) {
			int which = lmData->commonPrefixesCount;
			lmData->commonPrefixLens[which] +=
				snprintf(lmData->commonPrefixes[which],
			             sizeof(lmData->commonPrefixes[which]) - lmData->commonPrefixLens[which] - 1,
			             "%.*s",
			             dataLen,
			             data);
			if (lmData->commonPrefixLens[which] >= (int) sizeof(lmData->commonPrefixes[which])) {
		...
		if (!strcmp(elementPath, "ListMultipartUploadsResult/Upload")) {
			...
			if (lmData->uploadsCount == MAX_UPLOADS) {
				...
				initialize_list_multipart_upload(&(lmData->uploads[lmData->uploadsCount]));
			}
		}
		else if (!strcmp(elementPath, "ListMultipartUploadsResult/CommonPrefixes/Prefix")) {
			// Finished a Prefix
...
static S3Status listPartsXmlCallback(const char* elementPath, const char* data, int dataLen, void* callbackData)
{
	ListPartsData* lpData = (ListPartsData*) callbackData;
	int fit;
	if (data) {
...
#define safe_append(name, value)                                                                             \
	...
	ListMultipartData* lmData = (ListMultipartData*) malloc(sizeof(ListMultipartData));
	...
	lmData->responsePropertiesCallback = handler->responseHandler.propertiesCallback;
	lmData->listMultipartCallback = handler->responseXmlCallback;
	lmData->responseCompleteCallback = handler->responseHandler.completeCallback;
...
#define safe_append(name, value)                                                                             \
	...
	ListPartsData* lpData = (ListPartsData*) malloc(sizeof(ListPartsData));
	...
	lpData->responsePropertiesCallback = handler->responseHandler.propertiesCallback;
	lpData->listPartsCallback = handler->responseXmlCallback;
	lpData->responseCompleteCallback = handler->responseHandler.completeCallback;
