...
static S3Status errorXmlCallback(const char* elementPath, const char* data, int dataLen, void* callbackData)
{
	...
	if (!strcmp(elementPath, "Error")) {
		...
		errorParser->s3ErrorDetails.furtherDetails = errorParser->furtherDetails;
	}
	else {
		...
		if (errorParser->s3ErrorDetails.extraDetailsCount &&
		    !strcmp(elementName,
		            errorParser->s3ErrorDetails.extraDetails[errorParser->s3ErrorDetails.extraDetailsCount - 1].name))
		{
			// Append the value
		...
		if (errorParser->s3ErrorDetails.extraDetailsCount == sizeof(errorParser->extraDetails)) {
		...
		S3NameValue* nv = &(errorParser->extraDetails[errorParser->s3ErrorDetails.extraDetailsCount++]);
		nv->name = name;
		nv->value = value;
	}
...
S3Status error_parser_add(ErrorParser* errorParser, char* buffer, int bufferSize)
{
