...
{
	*requestContextReturn = (S3RequestContext*) malloc(sizeof(S3RequestContext));
...
S3Status S3_runall_request_context(S3RequestContext* requestContext)
{
	...
	do {
		...
		FD_ZERO(&exceptfds);
		int maxfd;
		S3Status status = S3_get_request_context_fdsets(requestContext, &readfds, &writefds, &exceptfds, &maxfd);
		...
		if (maxfd != -1) {
			...
			select(maxfd + 1, &readfds, &writefds, &exceptfds, (timeout == -1) ? 0 : &tv);
		}
		status = S3_runonce_request_context(requestContext, &requestsRemaining);
		if (status != S3StatusOK) {
			return status;
		}
	}
...
static S3Status process_request_context(S3RequestContext* requestContext, int* retry)
{
	...
	*retry = 0;
	...
	while ((msg = curl_multi_info_read(requestContext->curlm, &junk))) {
		...
		Request* request;
		if (curl_easy_getinfo(msg->easy_handle, CURLINFO_PRIVATE, (char**) (char*) &request) != CURLE_OK) {
			return S3StatusInternalError;
		}
		...
		if ((msg->data.result != CURLE_OK) && (request->status == S3StatusOK)) {
			request->status = request_curl_code_to_status(msg->data.result);
		}
		if (curl_multi_remove_handle(requestContext->curlm, msg->easy_handle) != CURLM_OK) {
			return S3StatusInternalError;
		}
...
S3Status S3_runonce_request_context(S3RequestContext* requestContext, int* requestsRemainingReturn)
{
	...
	do {
		status = curl_multi_perform(requestContext->curlm, requestsRemainingReturn);
	...
	while (s3_status == S3StatusOK && (status == CURLM_CALL_MULTI_PERFORM || retry));
...
S3Status S3_get_request_context_fdsets(S3RequestContext* requestContext,
                                       ...
                                       int* maxFd)
{
	return ((curl_multi_fdset(requestContext->curlm, readFdSet, writeFdSet, exceptFdSet, maxFd) == CURLM_OK)
	            ? S3StatusOK
