...
static S3Status convertBlsXmlCallback(const char* elementPath, const char* data, int dataLen, void* callbackData)
{
	...
	if (data) {
		if (!strcmp(elementPath,
		            ...
		            "TargetBucket"))
		{
			caData->targetBucketReturnLen += snprintf(&(caData->targetBucketReturn[caData->targetBucketReturnLen]),
			...
			if (caData->targetBucketReturnLen >= 255) {
				return S3StatusTargetBucketTooLong;
		                 ...
		                 "TargetPrefix"))
		{
			caData->targetPrefixReturnLen += snprintf(&(caData->targetPrefixReturn[caData->targetPrefixReturnLen]),
		            ...
		            "TargetGrants/Grant"))
		{
			...
			S3AclGrant* grant = &(caData->aclGrants[*(caData->aclGrantCountReturn)]);

			if (caData->emailAddress[0]) {
				grant->granteeType = S3GranteeTypeAmazonCustomerByEmail;
				strcpy(grant->grantee.amazonCustomerByEmail.emailAddress, caData->emailAddress);
			}
				...
				strcpy(grant->grantee.canonicalUser.displayName, caData->userDisplayName);
			}
			else if (caData->groupUri[0]) {
				if (!strcmp(caData->groupUri, ACS_GROUP_AWS_USERS)) {
					grant->granteeType = S3GranteeTypeAllAwsUsers;
				}
				else if (!strcmp(caData->groupUri, ACS_GROUP_ALL_USERS)) {
...
static S3Status convert_bls(char* blsXml,
                            char* targetBucketReturn,
                            char* targetPrefixReturn,
                            int* aclGrantCountReturn,
                            S3AclGrant* aclGrants)
...
static S3Status getBlsPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	GetBlsData* gsData = (GetBlsData*) callbackData;

	return (*(gsData->responsePropertiesCallback))(responseProperties, gsData->callbackData);
}

static S3Status getBlsDataCallback(int bufferSize, const char* buffer, void* callbackData)
{
	GetBlsData* gsData = (GetBlsData*) callbackData;
...
static void getBlsCompleteCallback(S3Status requestStatus, const S3ErrorDetails* s3ErrorDetails, void* callbackData)
{
	GetBlsData* gsData = (GetBlsData*) callbackData;

	if (requestStatus == S3StatusOK) {
		// Parse the document
		requestStatus = convert_bls(gsData->blsXmlDocument,
		                            gsData->targetBucketReturn,
		                            gsData->targetPrefixReturn,
		                            gsData->aclGrantCountReturn,
		                            gsData->aclGrants);
	}

	(*(gsData->responseCompleteCallback))(requestStatus, s3ErrorDetails, gsData->callbackData);
...
#define append(fmt, ...)                                                                     \
	do {                                                                                     \
		*xmlDocumentLenReturn += snprintf(&(xmlDocument[*xmlDocumentLenReturn]),             \
	...
	if (targetBucket && targetBucket[0]) {
		...
		append("<TargetPrefix>%s</TargetPrefix>", targetPrefix ? targetPrefix : "");
				...
				switch (grant->granteeType) {
					...
					default: // case S3GranteeTypeAllAwsUsers/S3GranteeTypeAllUsers:
						append("Group\"><URI>%s</URI>",
						       (grant->granteeType == S3GranteeTypeAllAwsUsers) ? ACS_GROUP_AWS_USERS
						                                                        : ACS_GROUP_ALL_USERS);
						break;
				}
...
static S3Status setSalPropertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	SetSalData* paData = (SetSalData*) callbackData;

	return (*(paData->responsePropertiesCallback))(responseProperties, paData->callbackData);
}
...
static int setSalDataCallback(int bufferSize, char* buffer, void* callbackData)
{
	...
	int remaining = (paData->salXmlDocumentLen - paData->salXmlDocumentBytesWritten);
	...
	memcpy(buffer, &(paData->salXmlDocument[paData->salXmlDocumentBytesWritten]), toCopy);
...
static void setSalCompleteCallback(S3Status requestStatus, const S3ErrorDetails* s3ErrorDetails, void* callbackData)
{
	SetSalData* paData = (SetSalData*) callbackData;

	(*(paData->responseCompleteCallback))(requestStatus, s3ErrorDetails, paData->callbackData);
                                  ...
                                  void* callbackData)
{
	if (aclGrantCount > S3_MAX_ACL_GRANT_COUNT) {
		(*(handler->completeCallback))(S3StatusTooManyGrants, 0, callbackData);
		return;
	}
	...
	S3Status status = generateSalXmlDocument(targetBucket,
	                                         targetPrefix,
	                                         aclGrantCount,
	                                         aclGrants,
	                                         &(data->salXmlDocumentLen),
	                                         data->salXmlDocument,
