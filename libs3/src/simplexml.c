...
static void saxStartElement(void* user_data, const xmlChar* nameUtf8, const xmlChar** attr)
{
	(void) attr;
	...
	if ((simpleXml->elementPathLen + len + 1) >= (int) sizeof(simpleXml->elementPath)) {
...
static void saxEndElement(void* user_data, const xmlChar* name)
{
	...
	simpleXml->status = (*(simpleXml->callback))(simpleXml->elementPath, 0, 0, simpleXml->callbackData);

	while ((simpleXml->elementPathLen > 0) && (simpleXml->elementPath[simpleXml->elementPathLen] != '/')) {
...
static void saxCharacters(void* user_data, const xmlChar* ch, int len)
{
	...
	simpleXml->status = (*(simpleXml->callback))(simpleXml->elementPath, (char*) ch, len, simpleXml->callbackData);
}
...
{
	if (!simpleXml->xmlParser && (!(simpleXml->xmlParser = xmlCreatePushParserCtxt(&saxHandlerG, simpleXml, 0, 0, 0))))
	{
		return S3StatusInternalError;
	}

	if (xmlParseChunk((xmlParserCtxtPtr) simpleXml->xmlParser, data, dataLen, 0)) {
