...
void response_headers_handler_add(ResponseHeadersHandler* handler, char* header, int len)
{
	...
	*end = 0;
	...
	if (!strncasecmp(header, "x-amz-request-id", namelen)) {
		responseProperties->requestId = string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, valuelen, fit);
	}
	else if (!strncasecmp(header, "x-amz-id-2", namelen)) {
		responseProperties->requestId2 = string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, valuelen, fit);
	}
	else if (!strncasecmp(header, "x-amz-storage-class", namelen) ||
	         !strncasecmp(header, "x-goog-storage-class", namelen))
	{
		responseProperties->xAmzStorageClass = string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, valuelen, fit);
	}
	else if (!strncasecmp(header, "x-amz-restore", namelen) || !strncasecmp(header, "x-goog-restore", namelen)) {
		responseProperties->xAmzRestore = string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, valuelen, fit);
	}
	else if (!strncasecmp(header, "Content-Type", namelen)) {
		responseProperties->contentType = string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, valuelen, fit);
	}
		...
		responseProperties->server = string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, valuelen, fit);
	}
	else if ((!strncasecmp(header, "ETag", namelen)) || (!strncasecmp(header, "Etag", namelen)))
	{ // some servers reply with Etag header
		responseProperties->eTag = string_multibuffer_current(handler->responsePropertyStrings);
		string_multibuffer_add(handler->responsePropertyStrings, c, valuelen, fit);
	}
	else if (!strncasecmp(header, S3_METADATA_HEADER_NAME_PREFIX, sizeof(S3_METADATA_HEADER_NAME_PREFIX) - 1)) {
		// Make sure there is room for another x-amz-meta header
		if (handler->responseProperties.metaDataCount == sizeof(handler->responseMetaData)) {
			return;
		}
		// Copy the name in
		char* metaName = &(header[sizeof(S3_METADATA_HEADER_NAME_PREFIX) - 1]);
		int metaNameLen = (namelen - (sizeof(S3_METADATA_HEADER_NAME_PREFIX) - 1));
		...
		if (!handler->responseProperties.metaDataCount) {
			handler->responseProperties.metaData = handler->responseMetaData;
		}

		S3NameValue* metaHeader = &(handler->responseMetaData[handler->responseProperties.metaDataCount++]);
...
void response_headers_handler_done(ResponseHeadersHandler* handler, CURL* curl)
{
	...
	// curl parse it
	time_t lastModified;
	if (curl_easy_getinfo(curl, CURLINFO_FILETIME, &lastModified) == CURLE_OK) {
