...
static S3Status xmlCallback(const char* elementPath, const char* data, int dataLen, void* callbackData)
{
	...
	if (data) {
		...
		else if (!strcmp(elementPath, "ListAllMyBucketsResult/Owner/DisplayName")) {
			string_buffer_append(cbData->ownerDisplayName, data, dataLen, fit);
		}
		else if (!strcmp(elementPath, "ListAllMyBucketsResult/Buckets/Bucket/Name")) {
			string_buffer_append(cbData->bucketName, data, dataLen, fit);
		}
		else if (!strcmp(elementPath, "ListAllMyBucketsResult/Buckets/Bucket/CreationDate")) {
		...
		if (!strcmp(elementPath, "ListAllMyBucketsResult/Buckets/Bucket")) {
			...
			S3Status status = (*(cbData->listServiceCallback))(
				cbData->ownerId, cbData->ownerDisplayName, cbData->bucketName, creationDate, cbData->callbackData);
...
static S3Status propertiesCallback(const S3ResponseProperties* responseProperties, void* callbackData)
{
	XmlCallbackData* cbData = (XmlCallbackData*) callbackData;
...
static void completeCallback(S3Status requestStatus, const S3ErrorDetails* s3ErrorDetails, void* callbackData)
{
	XmlCallbackData* cbData = (XmlCallbackData*) callbackData;
                     ...
                     void* callbackData)
{
	...
	XmlCallbackData* data = (XmlCallbackData*) malloc(sizeof(XmlCallbackData));
	if (!data) {
	...
	data->responsePropertiesCallback = handler->responseHandler.propertiesCallback;
