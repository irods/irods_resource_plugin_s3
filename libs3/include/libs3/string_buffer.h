...
#define string_buffer_append(sb, str, len, all_fit)                                              \
	do {                                                                                         \
		sb##Len += snprintf(&(sb[sb##Len]), sizeof(sb) - sb##Len - 1, "%.*s", (int) (len), str); \
...
#define string_multibuffer_current(smb) &(smb[smb##Size])

// Adds a new string to the string_multibuffer
#define string_multibuffer_add(smb, str, len, all_fit)                                                     \
	do {                                                                                                   \
		smb##Size += (snprintf(&(smb[smb##Size]), sizeof(smb) - smb##Size, "%.*s", (int) (len), str) + 1); \
