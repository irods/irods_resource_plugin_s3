        ...
        error_codes flush_cache_file(named_shared_memory_object& shm_obj) {
            ...
            while (true) {
                ...
                if (number_of_parts > 10000) {
                    rodsLog(LOG_WARNING, "Multiple timeouts resulted in a part size that is too small to upload the %ld size file.", cache_file_size);
                ...
                if (return_value == error_codes::UPLOAD_PART_TIMEOUT) {
                    ...
                    rodsLog(LOG_WARNING, "%s:%d (%s) [[%ld]] error in flushing cache file.  Trying a part size of %ld.",
                       ...
                       Function _func)
        {
            ...
            shm_obj.atomic_exec([this, &return_value, &shm_obj](auto& data) {
                ...
                if (object_status == object_s3_status::IN_S3 && this->download_to_cache_) {
                    rodsLog(config_.developer_messages_log_level, "%s:%d (%s) [[%lu]] Downloading object to cache", __FILE__, __LINE__, __FUNCTION__, this->get_thread_identifier());
                ...
                if (this->use_cache_) {
                    ...
                    if (!this->cache_fstream_ || !this->cache_fstream_.is_open()) {
                        ...
                        if (!cache_fstream_ || !cache_fstream_.is_open()) {
                            rodsLog(LOG_ERROR, "%s:%d (%s) [[%lu]] Failed to open cache file %s, error=%s open_mode: [app=%d][binary=%d][in=%d][out=%d][trunc=%d][ate=%d]",
